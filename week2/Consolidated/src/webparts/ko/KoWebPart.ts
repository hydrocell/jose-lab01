...
import KoViewModel, { IKoBindingContext } from './KoViewModel';

let _instance: number = 0;

export interface IKoWebPartProps {
  description: string;
}

export default class KoWebPart extends BaseClientSideWebPart<IKoWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: IKoBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        ...
        template: require('./Ko.template.html'),
        synchronous: false
      }
    );
  }
